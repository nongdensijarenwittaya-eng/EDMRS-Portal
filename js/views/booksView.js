/* ==========================================================================
   ...
   Renders books registry table & book detail drawer listing enclosed documents
   ========================================================================== */
      ...
      <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1.5rem;">
        <div>
          <h2 style="font-size: 1.4rem; font-weight: 700; color: var(--primary-950);">
            <i class="fa-solid fa-book-bookmark text-primary"></i> ทะเบียนเล่มเอกสาร
          </h2>
          <p style="font-size: 0.88rem; color: var(--text-muted);">
            การจัดหมวดหมู่เอกสาร ปพ. เป็นเล่ม รหัสเล่ม และการจัดเก็บตำแหน่งในคลัง
          </p>
        </div>
            ...
            <div class="card" style="margin-bottom: 0; position: relative; border-top: 4px solid var(--primary-600);">
              ...
              <div style="background: var(--bg-app); padding: 0.75rem; border-radius: var(--radius-md); margin-bottom: 1rem; font-size: 0.85rem;">
                ...
                <div style="display: flex; justify-content: space-between;">
                  <span style="color: var(--text-muted);">จำนวนรายการจัดเก็บ:</span>
                  <strong style="color: var(--primary-700);">${enclosedDocs.length} รายการ</strong>
                </div>
              </div>
                    ...
                    <tr>
                      ...
                      <td style="text-align: center;">
                        <div style="display: flex; gap: 0.35rem; justify-content: center;">
                          <button class="btn btn-danger btn-sm delete-book-doc-btn" data-id="${d.id}" data-code="${d.doc_code}" data-stdid="${d.student_id}" data-name="${d.student_name}">
          ...
          document.querySelectorAll('.delete-book-doc-btn').forEach(b => {
            b.onclick = () => {
              ...
              window.utils.confirmDialog(
                ...
                `คุณต้องการลบรายการเอกสาร <b>${docCode}</b> (${stdName || stdId}) ออกจากระบบใช่หรือไม่?`,
                async () => {
                  b.disabled = true;
                  try {
                    await window.db.deleteDocument(docCode, docId);
                    if (stdId) {
                      await window.db.deleteStudent(stdId);
                    }
                    window.utils.showToast('ลบรายการเอกสารเรียบร้อยแล้ว', 'success');
                    window.utils.closeModal();
                    this.refreshPage();
                  } catch (err) {
      ...
      btn.onclick = () => {
        ...
        window.utils.confirmDialog(
          ...
          async () => {
            btn.disabled = true;
            try {
              await window.db.deleteBook(code);
              window.utils.showToast('ลบทะเบียนเล่มเรียบร้อยแล้ว', 'success');
              this.refreshPage();
            } catch (err) {
              window.utils.showToast(`เกิดข้อผิดพลาดในการลบ: ${err.message}`, 'danger');
            }
        ...
        { text: 'ยกเลิก', class: 'btn btn-secondary' },
        {
          ...
          onClick: async () => {
            ...
            try {
              if (isEdit) {
                const updatedData = {
                  book_code: bookToEdit.book_code,
                  doc_type_code: type,
                  academic_year: year,
                  book_number: num,
                  start_no: start,
                ...
                await window.db.addBook(newBookData);
              ...
              if (modalSubmitBtn) { modalSubmitBtn.disabled = false; modalSubmitBtn.innerHTML = isEdit ? 'บันทึกการแก้ไข' : 'บันทึกเล่มเอกสาร'; }
            }
          }
        }
      ]
